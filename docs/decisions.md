...
### ADR-015: Langfuse Tracing via Per-Invocation CallbackHandler
**Date:** 2026-03-22
**Status:** accepted
**Context:** LangGraph traces (per-node spans, LLM generation spans with token counts and latency) needed to be visible in Langfuse for debugging and demo observability. The integration must be feature-gated and gracefully degrade when Langfuse is unavailable.
**Decision:** Add Langfuse's LangChain `CallbackHandler` as an optional tracing layer injected via `config["callbacks"]` at each graph invocation site (`chat.py`, `jobs.py`). A thin helper module (`observability/langfuse.py`) creates fresh per-request handler instances when `LANGFUSE_ENABLED=true`, returning empty config when disabled or when the `langfuse` package is not installed. Follows the existing try-import pattern from `observability/logging.py`. No changes to graph structure or node code.
**Consequences:** Per-invocation handler instances avoid async trace fragmentation (Langfuse GH #1926, #11516). The integration is zero-cost when disabled. Langfuse client is flushed on shutdown via `langfuse_shutdown()` in both API lifespan and worker `finally` blocks. Traces are enriched with `user_id` (patient_id) and `session_id` (thread_id) for filtering.
